...
const UserRegistrationDTO = Joi.object({
  ...
  password: Joi.string().regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@!#$%^&*])[a-zA-Z\d@!#$%^&*]{8,25}$/).required().messages({
    "string.pattern.base": "Password must have at least one lowcase letter, one uppercase letter, one digit, one special character and must be between 8 to 25 character long"
  }),
  confirmPassword: Joi.string().equal(Joi.ref('password')).required(),
  role: Joi.string().regex(/^(admin|employer|jobSeeker)$/).default('jobSeeker').messages({
    "string.pattern.base": "Role must be any of admin or employer or jobSeeker"
  }),
...
const ChangePasswordDTO = Joi.object({
  ...
  newPassword: Joi.string().regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@!#$%^&*])[a-zA-Z\d@!#$%^&*]{8,25}$/).required()
    .messages({
      "string.pattern.base": "Password must have at least one lowcase letter, one uppercase letter, one digit, one special character and must be between 8 to 25 character long",
    }),
